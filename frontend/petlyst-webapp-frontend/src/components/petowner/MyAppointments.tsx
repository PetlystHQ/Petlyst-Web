...
const MyAppointments: React.FC<MyAppointmentsProps> = ({
  ...
  useEffect(() => {
    if (externalAppointments) {
      console.log("Received external appointments:", externalAppointments.length);
  ...
  useEffect(() => {
    console.log("Applying filters and sorting to appointments:", appointments.length);
    ...
    filtered.sort((a, b) => {
      ...
      const dateTimeA = new Date(`${a.appointment_date}T${a.appointment_start_hour}`);
      const dateTimeB = new Date(`${b.appointment_date}T${b.appointment_start_hour}`);
  ...
  const fetchAppointments = async () => {
    ...
    try {
      console.log('Fetching appointments from /appointments/pet-owner endpoint');
      ...
      if (response.data && response.data.success) {
        ...
        console.warn('No appointments found or invalid response format:', response.data);
        setAppointments([]);
  ...
  const handleCancelAppointment = async (appointmentId: string) => {
    ...
    try {
      ...
      if (response.data) {
        ...
        const updatedFilteredAppointments = filteredAppointments.map(appointment => 
          appointment.appointment_id === appointmentId 
            ? { ...appointment, appointment_status: 'canceled' as 'canceled' } 
            : appointment
        );
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          >
            <span>Sort: {sortOrder === 'asc' ? 'Earliest First' : 'Latest First'}</span>
          </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-gray-700 mb-6">
              Are you sure you want to cancel your appointment at <span className="font-medium">{selectedAppointment.clinic_name}</span> on <span className="font-medium">{formatDate(selectedAppointment.appointment_date)}</span> at <span className="font-medium">{formatTime(selectedAppointment.appointment_start_hour)}</span>?
            </p>
                    ...
                    <tr key={`${appointment.appointment_id}-${index}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-center">
                        <div className="flex justify-center">
                          ...
                          {appointment.appointment_status === 'confirmed' && appointment.video_meeting && (
                            <button
          ...
          <div className="flex flex-col items-center justify-center">
            ...
            {statusFilter !== 'all' ? (
              <div className="text-center">
                <p className="text-gray-600 mb-3">You don't have any {statusFilter} appointments</p>
                <button 
              ...
              <p className="text-gray-600 text-center">
                Your appointment schedule is currently empty. You can schedule an appointment by visiting a clinic page.
              </p>
            )}
          </div>
        </div>
