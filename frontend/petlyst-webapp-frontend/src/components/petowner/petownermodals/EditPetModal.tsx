...
// Searchable dropdown component - Same as in AddPetModal
...
// Generate options for days, months, and years - same as AddPetModal
...
const EditPetModal: React.FC<EditPetModalProps> = ({ 
  ...
  useEffect(() => {
    if (pet && isOpen) {
      // Parse birth date from ISO format if available
      let birthDay = '';
      let birthMonth = '';
      let birthYear = '';
      
      if (pet.pet_birth_date) {
        try {
          const date = new Date(pet.pet_birth_date);
          if (!isNaN(date.getTime())) {
            birthDay = date.getDate().toString();
            birthMonth = (date.getMonth() + 1).toString(); // JavaScript months are 0-indexed
            birthYear = date.getFullYear().toString();
          }
        } catch (e) {
          console.error("Error parsing pet birth date:", e);
        }
      }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // Save the actual file object to formData.photo
      setFormData(prev => ({
        ...prev,
        photo: file,
        removePhoto: false
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!pet || !pet.pet_id) {
      setFormErrors({ submit: "No pet selected for editing" });
      return;
    }
    ...
    try {
      ...
      if (formData.photo instanceof File) {
        ...
        formDataObj.append('removePhoto', 'true');
      }
      
      // Send the update request
      const response = await axiosInstance.put(`/pets/${pet.pet_id}`, formDataObj, {
        headers: {
