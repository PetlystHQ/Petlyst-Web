...
const ClinicCard: React.FC<ClinicCardProps> = ({ clinic }) => {
  ...
  // Check if clinic is in favorites when component mounts
  useEffect(() => {
    const checkFavoriteStatus = async () => {
      if (!user || user.user_type !== 'pet_owner' || !token) {
        return;
      }
      
      try {
        const response = await axiosInstance.get(`/pet-owners/saved-clinics/${clinic.clinic_id}/check`);
  ...
  const handleFavoriteToggle = async (e: React.MouseEvent) => {
    ...
    // Only pet owners can add favorites
    if (user?.user_type !== 'pet_owner') {
      alert('Only pet owners can add clinics to favorites');
      return;
    }
    ...
    try {
      if (isFavorite) {
        ...
        const response = await axiosInstance.delete(`/pet-owners/saved-clinics/${clinic.clinic_id}`);
        if (response.data.success) {
          setIsFavorite(false);
        }
      } else {
        // Add to favorites
        const response = await axiosInstance.post(`/pet-owners/saved-clinics/${clinic.clinic_id}`, {});
        if (response.data.success) {
          setIsFavorite(true);
        }
      }
      ...
      <div className="rounded-lg overflow-hidden shadow-md transition-transform hover:shadow-lg hover:-translate-y-1 bg-white cursor-pointer h-full flex flex-col" onClick={handleCardClick}>
        ...
        <div className="border-t border-gray-100 px-4 py-3 bg-gray-50 flex justify-between items-center">
          ...
          <div className="flex space-x-2">
            {/* Only show favorite button if user is not a veterinarian */}
            {!isVeterinarian && (
              <button 
                className={`p-1.5 rounded-full transition-colors duration-150 ${isLoading ? 'cursor-not-allowed opacity-50' : 'hover:bg-gray-100'} ${isFavorite ? 'text-red-500' : 'text-gray-700'}`}
